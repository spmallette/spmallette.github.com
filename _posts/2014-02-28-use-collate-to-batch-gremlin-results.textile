...
gremlin> g.V.name.gather.transform{sql.rows("SELECT * FROM toy WHERE n IN ('" + it.join("','") + "')")}.next()
==> ...
{% endhighlight %}
...
{% highlight text %}
gremlin> g.V.name.toList().collate(2).transform{sql.rows("SELECT * FROM toy WHERE n IN ('" + it.join("','") + "')")}.scatter.toList()
==> ...
{% endhighlight %}
